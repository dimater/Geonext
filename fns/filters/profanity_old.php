...
class CensorWords
{
    ...
    */
    ...
    private function convert_unicode_callback($matches) {
        $char = $matches[0];

        if (mb_strlen($char, 'UTF-8') > 1) {
            return $char; // Keep multibyte characters as they are
        }

        $convertedChar = iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $char);

        return $convertedChar !== false ? $convertedChar : $char;
    }

    public function convert_unicode_text($text) {
        return preg_replace_callback('/./us', array($this, 'convert_unicode_callback'), $text);
    }
