...
if (isset($user[0])) {
            ...
            if ($custom_field['field_name'] === 'custom_field_1') {
                ...
                if ($custom_field['field_type'] === 'dropdown') {
                    ...
                    if (Registry::load('settings')->dateformat === 'mdy_format') {
                        $output['content'][$field_index]->field['value'] = date("M-d-Y", strtotime($custom_field['field_value']));
                    } else if (Registry::load('settings')->dateformat === 'ymd_format') {
                        $output['content'][$field_index]->field['value'] = date("Y-M-d", strtotime($custom_field['field_value']));
                    } else {
                        $output['content'][$field_index]->field['value'] = date("d-M-Y", strtotime($custom_field['field_value']));
                    }
    ...
    if (isset(Registry::load('config')->pro_version) && !empty(Registry::load('config')->pro_version)) {
        if (role(['permissions' => ['wallet' => 'view_users_wallet_balance']])) {
            ...
            $output['content'][5]->field['title'] = Registry::load('strings')->balance_wallet;
            $output['content'][5]->field['value'] = Registry::load('settings')->default_currency_symbol.' '.$user['wallet_balance'];
        }
    }

    if (role(['permissions' => ['profile' => 'view_email_address']])) {
        $output['content'][6] = new stdClass();
        $output['content'][6]->field['title'] = Registry::load('strings')->email_address;
        $output['content'][6]->field['value'] = $user['email_address'];
    }

    if (role(['permissions' => ['profile' => 'view_phone_number']])) {
        if (!empty($user['phone_number'])) {
            $output['content'][7] = new stdClass();
            $output['content'][7]->field['title'] = Registry::load('strings')->phone_number;
            $output['content'][7]->field['value'] = $user['phone_number'];

            if (!empty($user['phone_verified'])) {
                $output['content'][7]->field['title_img'] = Registry::load('config')->site_url.'assets/files/defaults/verified.png';
            }
        }
    ...
    if (role(['permissions' => ['profile' => 'view_profile_url']])) {
        ...
        $output['content'][8]->field['title'] = Registry::load('strings')->profile_url;
        $output['content'][8]->field['value'] = Registry::load('config')->site_url.$user['username'].'/';
        $output['content'][8]->field['link'] = $output['content'][8]->field['value'];
    }
    ...
    if (role(['permissions' => ['profile' => 'show_social_sharing_buttons']])) {
        ...
        $output['content'][9]->field['title'] = Registry::load('strings')->share_this_profile;
        $output['content'][9]->field['images'] = $social_share_icons;
        $output['content'][9]->field['class'] = 'social_share_icons';
    }
